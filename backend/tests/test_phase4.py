...
Test inventory (8 tests):
  1.  Missing ALL deductions → Warning anomaly emitted
...
# ---------------------------------------------------------------------------
# Test 1: Missing ALL mandatory deductions → Warning
# ---------------------------------------------------------------------------

def test_missing_all_deductions_emits_warning():
    """When income_tax, national_ins, and health are all None → Warning anomaly."""
    ...
    assert "ano_missing_mandatory_deductions" in ids, \
        f"Expected 'ano_missing_mandatory_deductions' in anomaly ids, got: {ids}"
    # Verify severity is Warning
    ano = next(a for a in anomalies if a.id == "ano_missing_mandatory_deductions")
    assert ano.severity.value == "Warning", \
        f"Expected severity Warning, got {ano.severity}"
    # Verify Hebrew content present
    assert "מס" in ano.what_we_found, "what_we_found should mention 'מס'"
...
def test_all_clean_no_extended_anomalies():
    ...
    extended_ids = {
        "ano_missing_mandatory_deductions",
